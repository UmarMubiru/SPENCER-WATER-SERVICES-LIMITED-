...
export default function ItemForm({ initial, onSubmit, submitLabel = "Save Item" }: Props) {
  ...
  }, [initial?.id, initial?.sku, initial?.barcode, initial?.name, initial?.description, initial?.category, initial?.unit, initial?.inventoryType, initial?.reorderLevel, initial?.unitCost, initial?.supplier, initial?.warehouse]);
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <select
              value={form.inventoryType ?? "MATERIAL"}
              onChange={(e) => update("inventoryType", e.target.value as "MATERIAL" | "COMPANY_TOOL")}
              className={inputClass}
            >

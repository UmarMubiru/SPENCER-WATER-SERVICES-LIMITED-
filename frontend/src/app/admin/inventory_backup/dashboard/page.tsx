...
export default function InventoryDashboardPage() {
  ...
  const applyFilters = () => {
    ...
    if (filters.status) {
      filtered = filtered.filter(item => {
        if (filters.status === 'low_stock') return item.status === 'LOW_STOCK';
        if (filters.status === 'in_stock') return item.status === 'IN_STOCK';
        return true;
      });
    }
  ...
  const fetchInventory = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/api/inventory/inventory-items/');
      if (response.ok) {
        ...
        const inStock = data.filter((item: InventoryItem) => item.status === 'IN_STOCK').length;
        const lowStock = data.filter((item: InventoryItem) => item.status === 'LOW_STOCK').length;
        const categories = [...new Set(data.map((item: InventoryItem) => item.category_name))].length;
  ...
  const handleArchive = async (id: string) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/inventory/items/${id}/archive/`, {
        method: 'POST',
      });
      if (response.ok) {
        fetchInventory();
  ...
  const handleUnarchive = async (id: string) => {
    if (!confirm('Are you sure you want to unarchive this item?')) return;
    
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/inventory/items/${id}/unarchive/`, {
        method: 'POST',
      });
      if (response.ok) {
        fetchInventory();
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-colors ${showArchived ? 'bg-orange-50 border-orange-300 text-orange-700 border' : 'bg-white border border-gray-300 hover:bg-gray-50'}`}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {[...new Set(inventory.map(item => item.category_name))].map(cat => (
                ...
                <tbody>
                  {filteredInventory.map((item) => (
                    <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-4 px-6 font-medium text-gray-900">{item.sku}</td>

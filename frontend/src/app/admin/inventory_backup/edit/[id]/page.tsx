...
export default function EditInventoryItemPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchItem = async () => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/inventory/items/${itemId}/`);
      if (response.ok) {
        const data = await response.json();
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        router.push(`/admin/inventory/details/${itemId}`);
      } else {
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                ...
                <Link href={`/admin/inventory/details/${itemId}`} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
                  Cancel
                </Link>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <div className="mt-6 flex justify-end gap-3">
                <Link
                  href={`/admin/inventory/details/${itemId}`}

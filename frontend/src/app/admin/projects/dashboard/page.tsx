...
export default function ProjectsDashboardPage() {
  ...
  const fetchProjects = async () => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/projects/?is_archived=${filters.isArchived}`);
      if (response.ok) {
  ...
  const handleArchive = async (id: string) => {
    if (!confirm('Archive this project? It will be hidden from the active list but not permanently removed.')) return;
  ...
  const handleUnarchive = async (id: string) => {
    if (!confirm('Unarchive this project? It will be restored to the active list.')) return;
    
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/projects/${id}/unarchive/`, {
        method: 'POST',
      });
      if (response.ok) {
        fetchProjects(); // Refresh the list
                ...
                <div 
                  ...
                  onClick={() => {
                    ...
                    if (stat.label === 'In Progress') {
                      ...
                      setFilters({ status: '', serviceLine: '', dateFrom: '', dateTo: '', isArchived: 'false' });
                    }
                  }}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFilters(prev => ({ ...prev, isArchived: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  >
                    ...
                    <tr key={project.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-4 px-6">
                        <div className="flex gap-3">
                          ...
                          <Link href={`/admin/projects/${project.project_reference}/schedule`} className="text-green-600 hover:text-green-800 font-medium">Update</Link>

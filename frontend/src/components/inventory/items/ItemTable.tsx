      ...
      <table className="min-w-full">
        <thead className="bg-blue-50/60">
          <tr>
            ...
            <th className="px-6 py-3.5 text-left text-[11px] font-semibold uppercase tracking-wide text-blue-500">Item Type</th>
        ...
        <tbody>
          {(items ?? []).map((item) => (
            <tr key={item.id} className="border-t border-blue-50 transition-colors hover:bg-blue-50/40">
              ...
              <td className="px-6 py-4"><span className={`rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-wide ${item.inventoryType === 'COMPANY_TOOL' ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-700'}`}>{item.inventoryType === 'COMPANY_TOOL' ? 'Company Tool' : 'Material'}</span></td>
              ...
              <td className="px-6 py-4 text-right">
                ...
                <Link href={`/admin/inventory/items/${item.id}/edit`} className="ml-3 text-blue-600 hover:underline">
                  Edit
                </Link>
              </td>
            </tr>
          ...
          {(!items || items.length === 0) && (
            <tr>
              <td colSpan={10} className="px-6 py-10 text-center text-blue-400">

...
export default function ItemForm({ initial, purchaseNumber, onSubmit, submitLabel = "Save Item" }: Props) {
  ...
  const [form, setForm] = useState<InventoryItemInput>({
    sku: initial?.sku ?? purchaseNumber ?? "",
    barcode: initial?.barcode ?? "",
    name: initial?.name ?? "",
    description: initial?.description ?? "",
    category: initial?.category ?? "",
    unit: initial?.unit ?? "pcs",
    inventoryType: initial?.inventoryType ?? "MATERIAL",
  ...
  }, [initial?.id, initial?.sku, initial?.barcode, initial?.name, initial?.description, initial?.category, initial?.unit, initial?.inventoryType, initial?.reorderLevel, initial?.unitCost, initial?.supplier, initial?.warehouse, purchaseNumber]);
  ...
  const itemPrice = Number(form.unitCost || 0);
  const grandTotal = itemPrice * Number(initialQuantity);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 grid gap-4 md:grid-cols-2">
          <div>
            ...
            <input
              readOnly
              value={form.sku || "Allocating purchase number…"}
          ...
          <div>
            ...
            <select value={form.inventoryType} onChange={(e) => update("inventoryType", e.target.value as "MATERIAL" | "COMPANY_TOOL")} className={inputClass}>
              <option value="MATERIAL">Material (consumed)</option>
              <option value="COMPANY_TOOL">Company Item / Tool (returned)</option>
            </select>
          </div>
      ...
      <button
        type="submit"
        disabled={submitting || (!isEditing && !form.sku)}
            ...
            >
        {submitting ? "Saving..." : !isEditing && !form.sku ? "Assigning Purchase No..." : submitLabel}
            </button>
            <button
              type="button"
              onClick={resetForm}
